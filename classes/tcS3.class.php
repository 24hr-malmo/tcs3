<?php
/**
 * File              : services/wordpress/src/wp-content/plugins/tcs3/classes/tcS3.class.php
...
class tcS3
{
    ...
    public function __construct()
    {
        ...
        $this->use_S3 = ($this->accessKey != "" && $this->accessSecret != "" && $this->get_bucket_name() != "" && $this->options["bucket_path"] != "" && $this->options["bucket_region"] != "");
    ...
    public function push_to_s3($keys, $localdir = false, $remotedir = false)
    {
        ...
        foreach ($keys as $key) {
            ...
            $uploader = new MultipartUploader($this->s3Client, $localFile, [
                'bucket' => $this->get_bucket_name($keys),
    ...
    public function get_bucket_name($keys = false) {
        $bucket = $this->options["bucket"];
        if ($keys != false && $keys[0] != '') {
            $bucket = apply_filters('tcs3/bucket_name', $bucket, $keys);
        }
        return $bucket;
    }

    public function get_s3_url($url = '') {
        $s3URL = $this->options["s3_url"];
        $s3URL = apply_filters('tcs3/s3_url', $s3URL, $url);
        return $s3URL;
    }
    ...
    public function delete_from_S3($keys, $dir = false, $bucketPath = true)
    {
        ...
        foreach ($keys as $key) {
            ...
            if ($this->s3Client->doesObjectExist($this->get_bucket_name($keys), $file)) {
    ...
    public function build_attachment_url($url, $dir = false)
    {
        ...
        if ($this->options["tcS3_fallback"] == 1) {
            ...
            $url = $this->get_s3_url($url) . $this->uploadDir . "/" . $matches[1];
        }
